...
const ObservationDetailsView = ({ 
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md border border-blue-100 mt-4 relative animate-fade-in">
            <button 
                onClick={onClose} 
                className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 font-bold text-xl px-2"
                title="Fermer"
            >
                ✕
            </button>
...
const ObservationForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // PATCH pour modification, POST pour création
    ...
    try {
      const res = await fetch(url, {
        method: method,
        headers: {
          // Format spécifique pour API Platform en mode PATCH
          'Content-Type': isEditMode ? 'application/merge-patch+json' : 'application/json',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-5 rounded-lg shadow-lg border-l-4 border-indigo-500 mt-4 relative animate-slide-down">
      ...
      <h3 className="font-bold text-lg text-indigo-900 border-b pb-3">
          {isEditMode ? '✏️ Modifier Observation' : '➕ Nouvelle Observation'}
      </h3>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
              <label className="block text-sm font-bold text-gray-700">Âge (Jours) <span className="text-red-500">*</span></label>
              <input type="number" required min="0" className="w-full border border-gray-300 p-2 rounded" value={data.age || ''} onChange={e => updateData('age', parseInt(e.target.value))} />
          </div>
          <div>
              <label className="block text-sm font-bold text-gray-700">Mortalité (Sujets) <span className="text-red-500">*</span></label>
              <input type="number" required min="0" className="w-full border border-gray-300 p-2 rounded bg-yellow-50" value={data.mortalite || ''} onChange={e => updateData('mortalite', parseInt(e.target.value))} />
          </div>
          <div>
              <label className="block text-sm font-bold text-gray-700">Poids Moyen (g) <span className="text-red-500">*</span></label>
              <input type="number" required step="0.01" min="0" className="w-full border border-gray-300 p-2 rounded" value={data.poidsMoyen || ''} onChange={e => updateData('poidsMoyen', parseFloat(e.target.value))} />
          </div>
          <div>
              <label className="block text-sm font-bold text-gray-700">Conso / Tête (g) <span className="text-red-500">*</span></label>
              <input type="number" required step="0.01" min="0" className="w-full border border-gray-300 p-2 rounded" value={data.consoTete || ''} onChange={e => updateData('consoTete', parseFloat(e.target.value))} />
          </div>
          <div className="bg-gray-50 p-2 rounded border border-gray-200">
              <label className="block text-sm font-bold text-gray-700">Type Aliment <span className="text-red-500">*</span></label>
              <select required className="w-full border border-gray-300 p-2 rounded bg-white mb-2" value={alimentSource} onChange={e => setAlimentSource(e.target.value)}>
              ...
              {alimentSource === 'Autres' && (
                  <div>
                      <label className="block text-xs font-bold text-indigo-700 mb-1">Précisez l'aliment : <span className="text-red-500">*</span></label>
                      <input type="text" required placeholder="Ex: Soja..." className="w-full border border-indigo-300 p-2 rounded bg-white text-sm" value={alimentPrecision} onChange={e => setAlimentPrecision(e.target.value)} />
                  </div>
              )}
          </div>
          <div>
              <label className="block text-sm font-bold text-gray-700">Mangeoires (Qté) <span className="text-red-500">*</span></label>
              <input type="number" required min="0" className="w-full border border-gray-300 p-2 rounded" value={data.mangeoires || ''} onChange={e => updateData('mangeoires', parseInt(e.target.value))} />
          </div>
          <div>
              <label className="block text-sm font-bold text-gray-700">Abreuvoirs (Qté) <span className="text-red-500">*</span></label>
              <input type="number" required min="0" className="w-full border border-gray-300 p-2 rounded" value={data.abreuvoirs || ''} onChange={e => updateData('abreuvoirs', parseInt(e.target.value))} />
          </div>
      </div>
      ...
      <div className="bg-gray-50 p-4 rounded border border-gray-200">
          ...
          <div className="flex gap-6 mb-3">
              <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="biosecurite" value="ok" required checked={data.biosecurite === 'ok'} onChange={() => updateData('biosecurite', 'ok')} className="w-4 h-4 text-green-600"/>
                  <span className="text-gray-800">Conforme</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="biosecurite" value="nok" required checked={data.biosecurite === 'nok'} onChange={() => updateData('biosecurite', 'nok')} className="w-4 h-4 text-red-600"/>
          ...
          <div>
              ...
              <textarea className="w-full border border-gray-300 p-2 rounded text-sm" placeholder="Observations sur l'hygiène..." rows={2} value={data.biosecuriteComment || ''} onChange={e => updateData('biosecuriteComment', e.target.value)} />
          </div>
      </div>
      ...
      <div className="space-y-4">
        <div><label className="block text-sm font-medium text-gray-700">Préoccupations du Client</label><textarea className="w-full border border-gray-300 rounded p-2" rows={2} value={common.concerns} onChange={e => setCommon({ ...common, concerns: e.target.value })} /></div>
        <div><label className="block text-sm font-medium text-gray-700">Observations Générales</label><textarea className="w-full border border-gray-300 rounded p-2" rows={2} value={common.observation} onChange={e => setCommon({ ...common, observation: e.target.value })} /></div>
        {/* CHAMP RECOMMANDATIONS AJOUTÉ ICI */}
        ...
        <div className="grid grid-cols-2 gap-4 bg-gray-50 p-3 rounded">
            <div><label className="text-xs text-gray-600">Effectif Départ</label><input type="number" className="w-full border p-1 rounded text-sm" value={data.effectifDepart || ''} onChange={e => updateData('effectifDepart', parseInt(e.target.value))} /></div>
        </div>
      </div>

      <div className="flex gap-3 justify-end pt-4">
          <button type="button" onClick={onCancel} className="px-5 py-2.5 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 font-medium">Annuler</button>
...
// --- COMPOSANT 4 : ITEM BÂTIMENT (ISOLÉ) ---
const BuildingItem = ({ building, visit, onRefresh }: { building: Building, visit: Visit, onRefresh: () => void }) => {
    ...
    const getExistingObservation = () => {
        if (!currentFlock?.observations || !visit) return null;
        return currentFlock.observations.find(obs => {
    ...
    const existingObs = getExistingObservation();

    useEffect(() => {
        setViewMode('summary');
    }, [isExpanded, currentFlock?.id]);
                ...
                <div className="p-5 bg-white">
                    {!currentFlock ? (
                        <div className="text-center py-6">
                            ...
                            {viewMode === 'create_flock' ? (
                                <NewFlockForm 
                                    customerIri={visit.customer['@id']}
                                    buildingIri={building['@id']}
                                    onSuccess={() => { setViewMode('summary'); onRefresh(); }}
                                    onCancel={() => setViewMode('summary')}
                                />
                            ) : (
                                !visit.closed && (
                                    <button 
                                        onClick={() => setViewMode('create_flock')}
                        ...
                        <div>
                            {viewMode === 'form' && (
                                <ObservationForm 
                                    visitIri={visit['@id']}
                                    flock={currentFlock}
                                    initialData={existingObs} 
                                    onSuccess={() => { setViewMode('summary'); onRefresh(); }}
                                    onCancel={() => setViewMode('summary')}
                                />
                            )}

                            {viewMode === 'details' && existingObs && (
                                    ...
                                    {existingObs ? (
                                        <div className="border border-blue-200 bg-blue-50 rounded-lg p-4">
                                            ...
                                            <div className="flex gap-3 pt-3 border-t border-blue-200">
                                                {!visit.closed && (
                                                    <button onClick={() => setViewMode('form')} className="flex-1 flex justify-center items-center gap-2 bg-white text-blue-700 border border-blue-300 py-2 rounded-lg font-bold hover:bg-blue-50 transition">✏️ Modifier</button>
                                                )}
                                        ...
                                        !visit.closed ? (
                                            <button onClick={() => setViewMode('form')} className="w-full py-6 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center text-gray-500 hover:border-indigo-500 hover:bg-indigo-50 hover:text-indigo-600 transition group">
