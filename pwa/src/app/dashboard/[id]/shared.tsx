...
export const generateExpertInsights = (obs: any, flock: any, benchmark: any, density: number, totalMortalite: number, dueVaccines: any[]) => {
    ...
    if (obs.data.abreuvoirs > 0) { 
        ...
        if (ratioAbr > 70) insights.push({ type: 'danger', text: `Manque d'abreuvoirs (1/${ratioAbr.toFixed(0)} suj).` }); 
    }
    if (obs.data.mangeoires > 0) { 
        const ratioMang = sujetsRestants / obs.data.mangeoires; 
        if (ratioMang > 50) insights.push({ type: 'danger', text: `Manque de mangeoires (1/${ratioMang.toFixed(0)} suj).` }); 
    }

    if (density > 22) {
        insights.push({ type: 'danger', text: `Surdensité critique : ${density} sujets/m² (Risque sanitaire).` });
    } else if (density > 18) {
        insights.push({ type: 'warning', text: `Densité élevée : ${density} sujets/m². Surveillez ventilation/litière.` });
    }
    ...
    if (benchmark) {
        if (benchmark.weightStatus === 'danger') 
            insights.push({ type: 'danger', text: `Retard Poids Critique (-${Math.abs(benchmark.weightGap).toFixed(0)}g).` });
        
        if (benchmark.feedStatus === 'danger' && benchmark.feedGap < 0) 
            insights.push({ type: 'danger', text: `Sous-consommation aliment (-${Math.abs(benchmark.feedGap).toFixed(0)}g/j).` });
    }
