    ...
    public function __invoke(PreOrder $order, EntityManagerInterface $em, TranslatorInterface $translator): JsonResponse
    {
        ...
        if ($order->getStatus() !== PreOrder::STATUS_CONFIRMED) {
            return new JsonResponse([
                'error' => $translator->trans('pre_order.deliver_only_from_confirmed', ['status' => $order->getStatus()])
            ], 400);
        }
        ...
        return new JsonResponse(['id' => $order->getId(), 'status' => PreOrder::STATUS_DELIVERED]);
    }
}
